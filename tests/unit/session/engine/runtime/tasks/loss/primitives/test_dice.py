...
def test_dice_loss_forward():
    ...
    # B=1, C=2, H=2, W=2
    # make logits favor class 1 at top-left and class 0 at others (perfect match)
...
def test_dice_loss_ignore_index():
    ...
    loss_module = dice_loss.DiceLoss(smooth=1.0, ignore_index=255)
    # B=1, C=2, H=2, W=2
    # logits favor class 0 at (0,0), class 1 at (0,1), class 0 at (1,0), class 1 at (1,1)
    logits = torch.tensor([[[[10.0, -10.0], [10.0, -10.0]],
                            [[-10.0, 10.0], [-10.0, 10.0]]]], dtype=torch.float32)
    ...
    # loss_2 should be higher than loss_1 because (1,1) is a mismatch in targets_mismatch,
...
def test_dice_loss_with_masks():
    ...
    logits = torch.tensor([[[[10.0, 10.0], [10.0, 10.0]],
                            [[-10.0, -10.0], [-10.0, -10.0]]]], dtype=torch.float32)
    # logits predict class 0 everywhere, targets is class 1 everywhere (complete mismatch)
