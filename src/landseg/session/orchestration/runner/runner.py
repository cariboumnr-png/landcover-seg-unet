...
class TrainingRunner:
    ...
    def run(self):
        ...
        Yields:
            dict:
                Metrics emitted at the end of each epoch.

        Behavior:
            - Processes phases sequentially
            - Consumes event streams generated by phase policies
            - Logs phase and epoch boundaries
            - Load and resume from the last checkpoint if configured
            - Persists checkpoints on request
            - Yields epoch metrics as a generator
            - Terminates early only in single-phase mode when early
            stopping is triggered

        Notes:
            - This method is a generator; execution is driven by iteration
            - Global termination (`StopRun`) is honored only when early
            stopping is enabled
        '''
