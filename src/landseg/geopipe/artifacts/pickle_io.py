...
# -------------------------------Public Function-------------------------------
def load_pickle_hash(fpath: str) -> tuple[int, str, typing.Any]:
    '''
    Helper to load a pickle config file.
    ...
    * `4`: pickle file found but probably corrupted
    999: other errors

    Returns:
        status, message, and loaded pickle object
    '''
    ...
    try:
        loaded = _load_pickle(fpath)
        file_hash_value = _get_hash(fpath)
        try:
            hash_record = _load_json(hash_path)
            if file_hash_value != hash_record.get(filename):
                return 1, f'Pickle hash value not matching with record: {hash_path}', loaded
            return 0, f'Pickle loaded with hash check successfully: {fpath} ', loaded
        except (FileNotFoundError, pickle.UnpicklingError):
            return 2, f'Hash record not found or corrupted: {hash_path}', loaded

    except FileNotFoundError:
        return 3, f'Pickle file not found: {fpath}', None

    except pickle.UnpicklingError:
        return 4, f'Pickle file not properly loaded: {fpath}', None

    return 999, 'Other errors occur during loading', None

def write_pickle_hash(
    fpath: str,
    src: list | dict | typing.Mapping | typing.Any
) -> None:
    '''Helper to write a pickle config file from a python dict or list.'''
    ...
    if root: # skip if write to root, e.g., ./file.pickle
        os.makedirs(root, exist_ok=True)

    # write src to pickle
    _write_pickle(fpath, src)

    # get src file hash
    hash_value = _get_hash(fpath)

    # load pickle with some error handling (create new if so)
